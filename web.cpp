        ...
        for(;;)
        {
            ...
            if(buffer.size() >= 3*16000) {
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    response["segmentNumber"] = i;
                    response["text"] = text;

                    // Send the JSON response back to the client
                    auto response_message = response.dump();
                    ws.text(true);
                    try {
                        ws.write(net::buffer(response_message));
                    } catch (const std::exception& e) {
                        std::cerr << "Failed to send message: " << e.what() << std::endl;
                        // Handle the error (e.g., by breaking the loop and/or closing the WebSocket)
                        break;
                    }
